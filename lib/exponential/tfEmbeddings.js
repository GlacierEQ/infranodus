...
class TFEmbeddingsService {
    ...
    computeEmbeddingSimilarity(embedding1, embedding2, metric = this.options.defaultSimilarityMetric) {
        ...
        const normA = Math.sqrt(embedding1.reduce((s, v) => s + v * v, 0));
        const normB = Math.sqrt(embedding2.reduce((s, v) => s + v * v, 0));

        switch (metric.toLowerCase()) {
            case 'euclidean': {
                const dist = Math.sqrt(embedding1.reduce((s, v, i) => s + (v - embedding2[i]) ** 2, 0));
                return 1 / (1 + dist);
            }
            case 'manhattan': {
                const dist = embedding1.reduce((s, v, i) => s + Math.abs(v - embedding2[i]), 0);
                return 1 / (1 + dist);
            }
            case 'dot': {
                const maxDot = normA * normB;
                return (dot + maxDot) / (2 * maxDot);
            }
            case 'cosine':
            default:
    ...
    async visualizeEmbeddings(texts, options = {}) {
        ...
        try {
            ...
            if (visOptions.method === 'pca') {
                ...
                await tsne.fit(inputTensor);
                reducedData = tsne.getOutputScaled();
            } else {
                reducedData = await inputTensor.array();
            }
...
module.exports = TFEmbeddingsService;
