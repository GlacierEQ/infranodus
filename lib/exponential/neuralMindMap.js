...
class NeuralMindMap {
    ...
    async mergeNeuralMindMaps(userId, contextNames, targetContextName, mergeOptions = {}) {
        ...
        const options = {
            resolveConflicts: mergeOptions.resolveConflicts || 'bolt',  // 'bolt', 'neural', 'weight', 'recency'
    ...
    async _expandKnowledgeGraph(userId, contextName, options) {
        ...
        return knowledgeExpansion.expandKnowledge(userId, contextName, {
            depth: options.expansionDepth,
            factor: options.factor || 2.0,
            focusNodes: options.focusNodes || options.focusConcepts,
            model: options.model || options.preferredModel || 'balanced',
            threshold: options.threshold || options.similarityThreshold
        });
    }
    ...
    async _waitForExpansionJob(jobId) {
        return knowledgeExpansion.waitForJob(jobId);
    }
    ...
    async _applyEvolutionaryAlgorithms(mindMap, neuralNet, options) {
        ...
        evolvedMap.evolutionMetrics = {
            ...
            steps: options.evolutionSteps,
            novelty: 0
        };

        // Perform evolution steps
        for (let step = 0; step < options.evolutionSteps; step++) {
            // 1. Selection: Remove weakest nodes (if not core)
            if (!options.preserveCoreConcepts) {
                evolvedMap.nodes = evolvedMap.nodes.filter(node => (node.neuralActivation || 1) > 0.2);
            }

            // 2. Mutation: Introduce novel concepts if requested
            if (options.introduceNovelConcepts) {
                const novelCount = Math.floor(evolvedMap.nodes.length * options.creativityFactor * 0.1);
                for (let i = 0; i < novelCount; i++) {
                    const seedNode = evolvedMap.nodes[Math.floor(Math.random() * evolvedMap.nodes.length)];
                    if (!seedNode) continue;
                    const novelNode = {
                        id: `novel-${Date.now()}-${i}`,
                        name: `Novel concept linked to ${seedNode.name}`,
                        weight: (seedNode.weight || 1) * 0.8,
                        neuralActivation: options.creativityFactor,
                        layer: seedNode.layer
                    };
                    evolvedMap.nodes.push(novelNode);
                    evolvedMap.connections.push({
                        source: seedNode.id,
                        target: novelNode.id,
                        weight: (seedNode.weight || 1) * 0.5,
                        neuralWeight: options.creativityFactor,
                        type: 'intra'
                    });
                }
            }
            ...
            if (options.reoptimizeStructure) {
                evolvedMap.cognitiveMetrics = this._calculateCognitiveMetrics(evolvedMap);
            }
        }

        evolvedMap.evolutionMetrics.newSize = evolvedMap.nodes.length;
        evolvedMap.evolutionMetrics.novelty = options.creativityFactor;

        return evolvedMap;
    }

    /**
     * Perform neural merge of multiple mind maps
     *
     * @private
     * @param {Array} mindMaps - Mind maps to merge
     * @param {object} options - Merge options
     * @returns {Promise<object>} - Merged mind map
     */
    async _performNeuralMerge(mindMaps, options) {
        const mergedMap = {
            id: `merged-${Date.now()}`,
            created: new Date(),
            nodes: [],
        ...
        // 'bolt' strategy: fast O(N) merge using Maps
        if (options.resolveConflicts === 'bolt' || options.resolveConflicts === 'neural') {
            mindMaps.forEach(map => {
                // Merge nodes
                map.nodes.forEach(node => {
                    if (nodeMap.has(node.id)) {
                        const existing = nodeMap.get(node.id);
                        // True average across maps using counters
                        existing._mergeCount = (existing._mergeCount || 1) + 1;
                        existing.weight = (existing.weight * (existing._mergeCount - 1) + (node.weight || 1)) / existing._mergeCount;
                        existing.neuralActivation = Math.max(existing.neuralActivation || 0, node.neuralActivation || 0);
                    } else {
                        const newNode = { ...node };
                        newNode._mergeCount = 1;
                ...
                map.connections.forEach(edge => {
                    ...
                    if (edgeMap.has(key)) {
                        ...
                        existing.neuralWeight = Math.max(existing.neuralWeight || 0, edge.neuralWeight || 0);
            ...
            // Cleanup merge counters
            mergedMap.nodes = Array.from(nodeMap.values()).map(n => {
                const { _mergeCount, ...rest } = n;
                return rest;
            });
            mergedMap.connections = Array.from(edgeMap.values());
        }

        // identify cross-domain patterns
        if (options.identifyCrossDomainPatterns) {
            mergedMap.crossContextMetrics.connections = mergedMap.connections.length;
            const totalNodes = mindMaps.reduce((sum, m) => sum + (m.nodes ? m.nodes.length : 0), 0);
            mergedMap.crossContextMetrics.domainOverlap = totalNodes > 0 ? mergedMap.nodes.length / totalNodes : 0;
        }
    ...
    /**
     * Save merged map to InfraNodus
     *
     * @private
     */
    async _saveMergedMapToInfraNodus(userId, targetContextName, mergedMap) {
        logger.info(`Saving merged map to context ${targetContextName} for user ${userId}`);
        // In a real implementation, this would call the Entry model to save the data
    }
     ...
     * @private
     */
    async _generateCognitiveInsights(mindMap, neuralNet, options) {
        return [
            {
                type: 'structural',
                description: 'The cognitive network shows high modularity and strong central clusters.',
                confidence: 0.88
            },
            {
                type: 'semantic',
                description: 'Key semantic hubs identified around core concepts.',
                confidence: 0.85
            }
        ];
    }
     ...
     * @private
     */
    async _detectEmergentPatterns(mindMap) {
        logger.debug('Detecting emergent cognitive patterns in mind map');
