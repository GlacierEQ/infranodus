...
class CollaborativeScaling {
    ...
    getSuggestions(collaborationId) {
        ...
        return collaboration.suggestionPool;
    }

    /**
     * Add data to the collaborative graph
     *
     * @private
     */
    _addToCollaborativeGraph(collaborationId, type, data, userId) {
        let graph = this.teamGraphs.get(collaborationId);
        if (!graph) {
            graph = { nodes: [], edges: [] };
            this.teamGraphs.set(collaborationId, graph);
        }

        if (type === 'node') {
            graph.nodes.push({ ...data, addedBy: userId, addedAt: new Date().toISOString() });
        } else {
            graph.edges.push({ ...data, addedBy: userId, addedAt: new Date().toISOString() });
        }

        logger.debug(`Added ${type} to collaborative graph ${collaborationId} by ${userId}`);
    }
}

module.exports = {
    CollaborativeScaling
};
