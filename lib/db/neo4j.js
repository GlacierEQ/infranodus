...
CypherQuery.addStatement = function(
    ...
    for (var sindex = 0; sindex < params['statements'].length; sindex++) {
        ...
        // Optimization: use a Set for O(1) lookup to avoid duplicates in Cypher MERGE query for Concepts
        var concepts_added = new Set()

        // Optimization: use a Set for O(1) lookup to avoid duplicates in Cypher MERGE query for Mentions
        var mentions_added = new Set()
        ...
        if (params['statements'][sindex]['concepts']) {
            ...
            for (var index = 0; index < concepts.length; index++) {
                ...
                if (!concepts_added.has(concepts[index])) {
                    concepts_exist[sindex] = index + 1

                    // Add into Set to check for duplicates in the next cycle
                    concepts_added.add(concepts[index])

                    // Create additional Cypher params map of concepts and statement IDs they belong to, just in case
                    // Optimization: We use the array from params directly for final storage, but the local Set for lookups
        ...
        if (params['statements'][sindex]['mentions']) {
            ...
            for (
                ...
                if (
                    ...
                    // Add the mention we just added into another Set, to check for duplicates in the next cycle
                    mentions_added.add(
                        params['statements'][sindex]['mentions'][puka]
                    )
    ...
    // Optimization: Removed expensive JSON.stringify logs on large graph data objects
    // as they cause massive performance degradation for large statements.
