...
function extractConcepts(
    ...
    // Optimization: convert stopwords_remove to a Set for O(1) lookups in filter
    ...
    // Optimization: convert stopwords to a Set for O(1) lookups later
    ...
    // Optimization: convert hashtags to a Set for O(1) lookups
    ...
    // Optimization: use stopwordsSet and hashtagsSet for O(1) lookups instead of indexOf
    ...
    for (var i = 0; i < conceptsclean.length; ++i) {
        ...
        // Optimization: use hashtagsSet for O(1) lookup
    ...
    // Optimization: use stopwordsSet and hashtagsSet for O(1) lookups
    for (var i = 0; i < lemmas.length; ++i) {
        if (lemmas[i][0] != undefined) {
            var firstLemma = lemmas[i][0]
            if (
                (!stopwordsSet.has(firstLemma) && firstLemma.length > 0) ||
                hashtagsSet.has(firstLemma)
            ) {
                lemmasclean.push(firstLemma.toLowerCase().replace(/[\\–-]/g, ''))
            }
        }
    }
