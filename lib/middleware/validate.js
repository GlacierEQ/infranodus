...
function extractConcepts(
    ...
    var stopwords_remove = new Set([''])

    for (var i = 0; i < stopwords_add.length; i++) {
        if (stopwords_add[i].charAt(0) == '-') {
            stopwords_remove.add(stopwords_add[i].substring(1))
            stopwords_add[i] = ''
        }
    }
    ...
    stopwords = stopwords.filter(function(el) {
        return !stopwords_remove.has(el)
    })

    // Optimization: convert stopwords to a Set for O(1) lookups in the main loops below
    ...
    // Optimization: convert hashtags to a Set for O(1) lookups in the main loops below
    ...
    // Optimization: replaced Array.indexOf with Set.has for O(1) performance in the loop
