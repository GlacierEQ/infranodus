...
Entry.getNodes = function(
    ...
    dbneo.cypherQuery(querynodes, function(err, nodes) {
        ...
        // Optimization: Use a Set for stopwords lookup O(1)
        var stopwords_set = new Set();
        for (var i = 0; i < stopwords_add.length; i++) {
            if (stopwords_add[i].charAt(0) != '-') {
                stopwords_set.add(stopwords_add[i]);
            }
        }

        // Optimization: Use a Map for context name lookup O(1)
        var contexts_map_to_name = new Map();
        for (var i = 0; i < contexts_map.length; i++) {
            contexts_map_to_name.set(contexts_map[i][1], contexts_map[i][0]);
        }

        // Optimization: Use a Map for node weighting O(1) instead of nested loop O(N*M)
        ...
        for (var i = 0; i < nodes_object.length; i++) {
            if (contexts_map_to_name.has(nodes_object[i][5])) {
                var weight = parseInt(nodes_object[i][7] || 3);

                // Source node
                var source_name = nodes_object[i][1];
                var source_uid = nodes_object[i][0];
                var existing_source = sorted_map.get(source_name);
                if (existing_source) {
                    if (existing_source.val == source_uid) {
                        existing_source.count += weight;
                    }
                } else if (!stopwords_set.has(source_name)) {
                    sorted_map.set(source_name, {
                        val: source_uid,
                        name: source_name,
                        count: weight
                    });
                ...
                var target_uid = nodes_object[i][2];
                var existing_target = sorted_map.get(target_name);
                if (existing_target) {
                    if (existing_target.val == target_uid) {
                        existing_target.count += weight;
                    }
                } else if (!stopwords_set.has(target_name)) {
                    sorted_map.set(target_name, {
                        val: target_uid,
                        name: target_name,
                        count: weight
                    });
        ...
        sorted = Array.from(sorted_map.values());
        ...
        // Optimization: Use a Set for top nodes ID lookup O(1)
        var top_nodes_ids = new Set(sorted.map(n => n.val));
        ...
        for (var i = 0; i < nodes_object.length; i++) {
            if (contexts_map_to_name.has(nodes_object[i][5])) {
                ...
                // Optimization: Use O(1) Set lookup instead of O(M) linear search
                if (top_nodes_ids.has(nodes_object[i][0]) && top_nodes_ids.has(nodes_object[i][2])) {
                    ...
                    let current_context_name = contexts_map_to_name.get(nodes_object[i][5]);
