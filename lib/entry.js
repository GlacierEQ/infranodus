...
Entry.getNodes = function(
    ...
    // Optimization: Create a Map for O(1) lookup of context UID by name
    // This avoids linear searches in contexts_map during Cypher query construction
    var contextsMapByName = new Map()
    for (var i = 0; i < contexts_map.length; i++) {
        contextsMapByName.set(contexts_map[i][0], contexts_map[i][1])
    }
    ...
    if (contexts.length > 0 && contexts[0]) {
        for (var u = 0; u < contexts.length; ++u) {
            ...
            querynodes +=
                ...
                contextsMapByName.get(contexts[u]) +
                "') "
            querynodes +=
                ' YIELD rel, start, end ' +
                view_filter +
    ...
    dbneo.cypherQuery(querynodes, function(err, nodes) {
        ...
        for (var i = 0; i < stopwords_add.length; i++) {
            if (stopwords_add[i].charAt(0) == '-') {
                ...
                stopwordsSet.add(stopwords_add[i])
            }
        }

        // Optimization: create a Map from contexts_map for O(1) context lookups
        var contextMapByUid = new Map()
        for (var k = 0; k < contexts_map.length; k++) {
            contextMapByUid.set(contexts_map[k][1], contexts_map[k][0])
        }

        // Optimization: use a Map to track node weights during the first pass instead of linear searches in an array
        var sortedMap = new Map() // name -> {val, name, count}
        ...
        for (var i = 0; i < nodes_object.length; i++) {
            ...
            if (contextName) {
                var sourceId = nodes_object[i][0]
                var sourceName = nodes_object[i][1]
                var targetId = nodes_object[i][2]
                var targetName = nodes_object[i][3]
                var weight = parseInt(nodes_object[i][7] || 3)

                // Source node processing
                var sourceNode = sortedMap.get(sourceName)
                if (!sourceNode) {
                    if (!stopwordsSet.has(sourceName)) {
                        sortedMap.set(sourceName, {
                            val: sourceId,
                            name: sourceName,
                            count: weight,
                        })
                    }
                } else if (sourceNode.val === sourceId) {
                    sourceNode.count += weight
                }

                // Target node processing
                var targetNode = sortedMap.get(targetName)
                if (!targetNode) {
                    if (!stopwordsSet.has(targetName)) {
                        sortedMap.set(targetName, {
                            val: targetId,
                            name: targetName,
                            count: weight,
                        })
                    }
                } else if (targetNode.val === targetId) {
                    targetNode.count += weight
                }
            }
        }

        // Convert Map values to array for sorting
        var sorted = Array.from(sortedMap.values())
        ...
        // Optimization: create a Set of top node UIDs for O(1) source/target checks in the second pass
        var topNodesUids = new Set()
        for (var j = 0; j < sorted.length; j++) {
            topNodesUids.add(sorted[j].val)
        }
        ...
        for (var i = 0; i < nodes_object.length; i++) {
            ...
            if (contextName) {
                ...
                if (topNodesUids.has(sourceId) && topNodesUids.has(targetId)) {
                    var sourceName = nodes_object[i][1]
                    var targetName = nodes_object[i][3]
                    var edgeId = nodes_object[i][4]
                    var statementId = nodes_object[i][6]
                    var weight = parseInt(nodes_object[i][7] || 3)
                    ...
                    if (edges_added[edgeKey]) {
                        // We have this same edge already?
                        if (edges_added[edgeKey].context_matrix[contextName]) {
                            // Add another statement to that edge
                            edges_added[edgeKey].context_matrix[contextName][
                                statementId
                            ] = weight
                            edges_added[edgeKey].weight += weight
                        }
                        // We have an edge connecting these nodes but the context is different
                        else {
                            // Let's add the new context
                            edges_added[edgeKey].context_matrix[
                                contextName
                            ] = {}
                            edges_added[edgeKey].context_matrix[contextName][
                                statementId
                        ...
                        var context_statement = {}
                        context_statement[contextName] = {}
                        context_statement[contextName][statementId] = weight
