...
Entry.getNodes = function(
    ...
    const contextsMapByName = new Map(contexts_map)
    ...
    if (contexts.length > 0 && contexts[0]) {
        for (var u = 0; u < contexts.length; ++u) {
            ...
            // Using Map for O(1) lookup of context UID by name
            const contextUid = contextsMapByName.get(contexts[u])
            ...
            context_query += " rel.context='" + contextUid + "' "
